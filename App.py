...
def add_STUDENTS(root, db):
    def add_query():
        ...
        if (len(prn) == 0):
            tkinter.messagebox.showwarning("WARNING", "All fields are compulsory(Except: Mobile number)")
            return
        if (len(name) == 0):
            tkinter.messagebox.showwarning("WARNING", "All fields are compulsory(Except: Mobile number)")
            return
        ...
        if len(mobile) == 0 and db.students.count_documents({'PRN': prn}, limit=1) == 0:
            ...
            tkinter.messagebox.showwarning("ERROR", "STUDENT Already Exists")
            return

        newwin.destroy()
        tkinter.messagebox.showinfo("Add Student", "Student Added")
...
def del_data(root, db):
    def delete():
        ...
        prn = E1.get()
        if (len(prn) == 0):
            tkinter.messagebox.showwarning("WARNING", "Enter a Valid PRN")
            return
        if db.students.count_documents({'PRN': prn}, limit=1) == 0:
            tkinter.messagebox.showwarning("ERROR", "STUDENT Does Not Exist")
            return
        else:
            db.students.delete_one({'PRN': prn})
        newwin.destroy()
        tkinter.messagebox.showinfo("Delete Student", "Student Deleted")
...
def update_data(root, db):
    def UPDD():
        ...
        newwin.destroy()
        tkinter.messagebox.showinfo("Update Student", "Student Updated")
